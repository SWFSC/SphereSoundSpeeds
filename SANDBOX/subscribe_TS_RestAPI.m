...
endpoint = '/api/sounder/data-output/ts-detection-chirp-subscriptions';
...
% % Single Target subscription
% json_body = [ ...
%     '{', newline, ...
%     '   "channel-id": "' channelIDs{3} '",', newline, ...
%     '   "settings": {', newline, ...
%     '     "range": 8,', newline, ...
%     '     "range-start": 1,', newline, ...
%     '     "layer-type": "pelagic",', newline, ...
%     '     "min-ts-value": -90,', newline, ...
%     '     "min-echo-length": 0.8,', newline, ...
%     '     "max-echo-length": 1.8,', newline, ...
%     '     "min-spacing": 0,', newline, ...
%     '     "max-phase-deviation": 20,', newline, ...
%     '     "max-gain-compensation": 3,', newline, ...
%     '     "format-type": "EK60"', newline, ...
%     '   },', newline, ...
%     '   "subscription-name": "MyTSSub",', newline, ...
...
json_body = [ ...
    ...
    '   "settings": {', newline, ...
    '     "range": 3,', newline, ...
    '     "range-start": 3,', newline, ...
    '     "layer-type": "pelagic",', newline, ...
    '     "min-ts-value": -70,', newline, ...
    '     "range-before-target": 0.15,', newline, ...
    '     "range-after-target": 0.35,', newline, ...
    '     "max-gain-compensation": 3,', newline, ...
    '     "max-phase-deviation": 20,', newline, ...
    '     "format-type": "HAC"', newline, ...
    '   },', newline, ...
    '   "subscription-name": "MyTSChirpSub",', newline, ...
    '   "subscriber-name": "MatlabClient"', newline, ...
    '}' ...
];
...
topic = 'Tsdc.v1';      % Topic filter for TS Detection Chirp (C-struct)
% topic = 'Tsd.v1';      % Topic filter for TS Detection Chirp (C-struct)
...
try
    for i = 1:5
        ...
        if ~isempty(dataFrame)
            ...
            % Parse Header
            ts_data.datagram_id   = typecast(dataFrame(1:4),   'uint32');
            ts_data.datagram_size = typecast(dataFrame(5:8),   'uint32');
            ts_data.ping_number   = typecast(dataFrame(9:12),  'uint32');
            ts_data.ping_time     = typecast(dataFrame(13:20), 'uint64');

            ntp_epoch_offset = datetime(1601, 1, 1);
            seconds_since_epoch = double(ts_data.ping_time) / 1e7;
            ts_data.datetime = ntp_epoch_offset + seconds(seconds_since_epoch);

            % numTraces = typecast(dataFrame(21:22),   'uint16');
            % fprintf('numTraces = %d\n', numTraces)

            % ts_data.data_source   = char(dataFrame(21:80)); % char[60]
            ts_data.channel_id  = strtrim(char(dataFrame(21:80)));

            num_targets = typecast(dataFrame(81:82), 'uint16');
